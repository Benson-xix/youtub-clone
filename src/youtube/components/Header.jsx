// rafc
import { Box, Circle, Flex, HStack, Icon, IconButton, LinkBox, Badge, Avatar, Link } from '@chakra-ui/react'
import React, { useEffect, useState as usePost } from 'react'
import {IoSearchOutline, IoAppsOutline, } from "react-icons/io5"
import {IoMdNotificationsOutline} from "react-icons/io"
import {FaMicrophone} from "react-icons/fa"
import {RiVideoAddLine} from "react-icons/ri"
import {useLocation} from "react-router-dom"

import avatericon from "../assets/img/icon1.png"
...
const changeCurrentTag = (item) => {
  ...
  setTags(newItems)
    ...
    const [writeItems, setWriteItems] = usePost([]);
    const [write, setWrite] = usePost("");
...
<Flex alignItems={"center"} justifyContent={"space-between"} w={"100%"} py={2} pr={10}>
        ...
        <Flex maxW={600}  flex={1} alignItems={"center"}> 
            ...
            <Circle ml={3} w={45} h={45} bgColor={"gray.900"} cursor={"pointer"} rounded={"full"} >
                <FaMicrophone color={"#0febbb"} />
            </Circle>
         </Flex>
         ...
         <HStack>
          ...
          <RiVideoAddLine size={20} color={"#fff"}/>
           </IconButton>


           <IconButton bgColor={"transparent"} _hover={{bgColor:"gray.900"}} >
            <IoAppsOutline  size={20} color={"#fff"}/>
           </IconButton>
           ...
           <Box position={"relative"}>
            <Avatar src={avatericon} width={30} cursor={"pointer"}height={30} />
           </Box>
...
<HStack className={"scrollbar"} overflowX={ "scroll"}py={3} px={5} borderTop={1} borderStyle={"solid"} borderBottom={1} borderColor={"gray.600"}>
{
tags.map ((item, index) => (
  <LinkBox key={index} onClick={() => changeCurrentTag(item)}>
      <Link to={"item.link"}>
                <Badge  border={1} bgColor={item.isActive ? "white" : "gray.600"}  color={item.isActive ? "black" : "white"}borderStyle={"solid"} borderColor={"gray.500"} rounded={"full"} py={2} px={4}>
                    {item.name}
                  </Badge>
      </Link>
</LinkBox>
